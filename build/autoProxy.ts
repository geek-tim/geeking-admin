...
export default function createServiceProxyPlugin(
    options: ServiceProxyPluginOptions,
) {
    const {
        serviceConfig,
        proxyPrefix = 'proxy-',
    ...
    return {
        ...
        config(
            config: UserConfig,
            { mode, command }: { mode: string; command: 'build' | 'serve' },
        ) {
            ...
            if (!enableProxy || !isDev) {
                ...
                if (envConfig) {
                    ...
                    console.warn(
                        `[auto-proxy] 已加载 ${Object.keys(envConfig).length} 个服务地址`,
                    )
                } else {
            ...
            console.warn(
                `[auto-proxy] 已加载${mode}模式 ${Object.keys(serviceConfig[mode]).length} 个服务地址`,
            )

            const { proxyConfig, proxyMapping } =
                generateProxyFromServiceConfig(serviceConfig, mode, proxyPrefix)
...
function generateProxyFromServiceConfig(
    ...
    try {
        ...
        if (!envConfig) {
            console.warn(
                `[auto-proxy] 未找到环境 "${mode}" 的配置，使用 development 配置`,
            )
...
function generateProxyFromConfig(
    ...
    Object.entries(envConfig).forEach(([serviceName, serviceUrl]) => {
        if (typeof serviceUrl === 'string' && serviceUrl.trim()) {
            ...
            const isWs =
                serviceUrl.startsWith('ws://') ||
                serviceUrl.startsWith('wss://')
            // 生成代理配置
            proxyConfig[proxyPath] = {
                ...
                rewrite: (path: string): string =>
                    path.replace(
                        new RegExp(`^/${proxyPrefix}${serviceName}`),
                        '',
                    ),
            }
