...
const fileURL = ref(
    'https://images.unsplash.com/photo-1663529628961-80aa6ebcd157?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=764&q=80',
)

const { loading, downloading, abort, send, data } = useRequest(
...
const downloadProcess = computed(() => {
    if (!downloading.value.loaded) return 0
    return Math.floor(
        (downloading.value.loaded / downloading.value.total) * 100,
    )
})
