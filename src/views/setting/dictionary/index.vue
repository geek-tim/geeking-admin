...
const {
    bool: dictLoading,
    setTrue: startDictLoading,
    setFalse: endDictLoading,
} = useBoolean(false)
const {
    bool: contentLoading,
    setTrue: startContentLoading,
    setFalse: endContentLoading,
} = useBoolean(false)
...
const dictColumns: DataTableColumns<Entity.Dict> = [
    ...
    },
    {
        ...
        key: 'code',
        render: (row) => {
            return <CopyText value={row.code} />
        },
    },
    {
        title: '操作',
            ...
            return (
                <NFlex justify="center">
                    <NButton
                        size="small"
                        onClick={() =>
                            dictContentRef.value!.openModal('edit', row)
                        }
                    >
                        编辑
                    </NButton>
        ...
        <div class="flex-1">
            <n-card>
                <template #header>
                    <NButton
                        type="primary"
                        :disabled="!lastDictCode"
                        @click="dictContentRef!.openModal('add')"
                    >
                        <template #icon>
                            <icon-park-outline-add-one />
                        </template>
                ...
                <template #header-extra>
                    <NFlex>
                        <NButton
                            ...
                            :disabled="!lastDictCode"
                            secondary
                            @click="getDictContent(lastDictCode)"
                        >
                            <template #icon>
                                <icon-park-outline-refresh />
                            </template>
